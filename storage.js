...
import {COURSE_VERSION} from './levels.js';
const ROOT='roomTiltGame.';
export const DEFAULT_SETTINGS=Object.freeze({strongGravity:true,dynamicCamera:true,sound:true,haptic:true,smoothing:.09,deadZone:.055,sensitivity:1,invertX:false,invertZ:false,quality:'auto',compactHUD:true,effects:true,volume:.65});
export function normalizeSettings(value={},reducedMotion=false) {
  ...
  for(const [key,min,max] of [['smoothing',0,.3],['deadZone',0,.25],['sensitivity',.4,2.5],['volume',0,1]]){
...
export function normalizeProgress(value,count=22) {
  const fresh={version:2,courseVersion:COURSE_VERSION,legacyBestTimes:Array(count).fill(null),current:0,unlocked:1,completed:[],bestTimes:Array(count).fill(null),attempts:Array(count).fill(0),restarts:Array(count).fill(0)};
  ...
  for(let i=0;i<count;i++){
    const old=value.legacyBestTimes?.[i];if(typeof old==='number'&&Number.isFinite(old)&&old>0&&old<86400)fresh.legacyBestTimes[i]=old;
...
export class SaveStore {
  constructor(adapter=null,count=22,reducedMotion=false,onError=()=>{}) {
    ...
    if(value.courseVersion!==COURSE_VERSION){
      this.progress.legacyBestTimes=this.progress.bestTimes.map((t,i)=>t??this.progress.legacyBestTimes[i]);
      this.progress.bestTimes.fill(null);
    }
    this.courseMigrated=value.courseVersion!==COURSE_VERSION&&this.progress.legacyBestTimes.some(Number.isFinite);
    const old={};
  ...
  resetProgress(){this.progress=normalizeProgress(null,this.count);this.courseMigrated=false;this.persist();}
}
