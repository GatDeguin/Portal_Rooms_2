...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--filter',default='',help='Render only named fixtures; default renders all');args=parser.parse_args()
    OUT.mkdir(parents=True,exist_ok=True);library=find_library('EGL')
    ...
    context=ef('eglCreateContext',P,[P,P,P,C.POINTER(I)])(display,config,None,(I*3)(0x3098,2,0x3038));surface=ef('eglCreatePbufferSurface',P,[P,P,C.POINTER(I)])(display,config,(I*5)(0x3057,512,0x3056,512,0x3038))
    ...
    def shader(kind,source):
        ...
        return ident
    packet=json.loads(subprocess.check_output(['node','tests/export-render-fixtures.mjs'],cwd=ROOT,text=True))
    ...
    programs={};vs=shader(0x8B31,packet['vertex'])
    for tier,source in packet['fragments'].items():
        fs=shader(0x8B30,source);program=create_program();attach(program,vs);attach(program,fs);link(program);status=I();program_iv(program,0x8B82,C.byref(status))
        if not status.value:
            buf=C.create_string_buffer(32768);program_log(program,len(buf),None,buf);raise AssertionError(buf.value.decode())
        programs[tier]=program;print('PASS compile/link',tier,flush=True)
    ...
    setters={name:gf('gl'+name[0].upper()+name[1:],None,[I]+[F]*n) for name,n in [('uniform1f',1),('uniform2f',2),('uniform4f',4)]};results=[]
    for item in packet['fixtures']:
        if args.filter and args.filter not in item['name']:continue
        ...
        image.save(OUT/(item['name']+'.png'));results.append({'name':item['name'],'size':[width,height],'rgb_stddev':stat.stddev});print('PASS draw',item['name'],flush=True)
    (OUT/'report.json').write_text(json.dumps({'status':'passed','backend':'offscreen OpenGL ES, not browser WebGL','filter':args.filter,'renderer':renderer,'version':version,'compiled_profiles':list(programs),'draws':results},indent=2))
