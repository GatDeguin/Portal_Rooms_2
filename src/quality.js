...
export const TIERS=Object.freeze({
  ...
  cinematic:{pixels:3200000,dpr:2,steps:176,shadows:64,ao:8,reflections:56,fog:12,octaves:7,detail:3,gi:3,reflectionSamples:2,portalLayers:6}
});
export function drawingSize(width,height,dpr=1,tier='medium',scale=1){
  ...
  const density=Math.min(q.dpr,Math.max(.5,finite(dpr,1)))*clamp(finite(scale,1),.5,1);
  const ratio=Math.min(density,Math.sqrt(q.pixels/(w*h)));
...
export class AdaptiveQuality {
  ...
  sample(ms,active=true){
    ...
    if(this.total<2000||this.frames<10)return false;
    this.average=this.total/this.frames;this.total=0;this.frames=0;
    const old=`${this.tier}:${this.scale}`;
    this.slowWindows=this.average>28?this.slowWindows+1:0;
    this.fastWindows=this.average<18.5?this.fastWindows+1:0;
    if(this.mode==='auto'){
      if(this.slowWindows>=1){
        ...
        else if(this.tier!=='high'){this.tier=this.tier==='low'?'medium':'high';this.scale=.8;}
        this.fastWindows=0;
      }
    }
