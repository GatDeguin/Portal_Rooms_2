...
test('all shader quality variants retain the original perspective camera',()=>{for(const tier of ['low','medium','high','cinematic']){const src=fragmentShader(tier);assert.match(src,/mix\(5\.78,5\.08,land\)/);assert.match(src,/float fov=mix\(1\.02,1\.34,land\)/);}});
