...
def main():
    ...
    (OUT/'report.json').write_text(json.dumps({'status':'passed','backend':'offscreen OpenGL ES, not browser WebGL','renderer':renderer,'version':version,'compiled_profiles':list(programs),'draws':results},indent=2))
