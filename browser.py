...
def offline_html(instrument=False):
    imports={}
    for path in (ROOT/'src').glob('*.js'):
        source=re.sub(r"from (['\"])\./([^'\"]+)\1",r"from 'portal/\2'",path.read_text())
        if instrument and path.name=='app.js':source+='\nwindow.__engine=engine;window.__feedback=feedback;'
...
def main():
    ...
    def shot(page,name):page.wait_for_timeout(550);page.screenshot(path=str(OUT/f'{name}.png'));shots.append(name+'.png')
    ...
    try:
      with sync_playwright() as p:
        ...
        page.locator('#startLevelsBtn').click();phase(page,'selector');check('selector lists 22 inspectable rooms and marks 21 locked entrances',page.locator('#levelGrid button').count()==22 and page.locator('#levelGrid button[data-locked=true]').count()==21 and page.locator('#previewPlayBtn').is_enabled())
        ...
        page.locator('#pauseDialog [data-action=settings]').click();phase(page,'settings');page.locator('#sensitivity').evaluate("e=>{e.value='1.7';e.dispatchEvent(new Event('input',{bubbles:true}));}");page.locator('#invertX').check();page.locator('[data-settings-tab=scene]').click();page.locator('#quality').select_option('low');page.locator('[data-settings-tab=controls]').click()
